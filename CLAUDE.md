...
Next.js 16 application bootstrapped with Create Next App. Uses React 19, TypeScript, and Tailwind CSS v4.
...
- `app/layout.tsx` — Root layout with font config and metadata
- `app/globals.css` — Tailwind v4 import + CSS custom properties for light/dark theming
- `public/` — Static assets
- Path alias: `@/*` maps to project root

### Styling

Tailwind CSS v4 uses the new `@import "tailwindcss"` syntax in `globals.css`. Dark mode is handled via `prefers-color-scheme` media query with CSS custom properties (`--background`, `--foreground`).
...
- ALWAYS write the plan to a `.md` file in the project root (e.g., `PLAN.md` or `plan-feature-name.md`). Never put plans in subdirectories.
...
Server Components by default (App Router). Mark client components with `"use client"` directive when needed.
